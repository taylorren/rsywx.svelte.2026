<script lang="ts">
	...
	const items = $derived([
		{ label: '已读', value: fmt(summary?.books_read), icon: '📖' },
		{ label: '书评', value: fmt(summary?.reviews_written), icon: '✒️' },
		{
			label: '书评率',
			value:
				summary && summary.books_read > 0
					? `${Math.round((summary.reviews_written / summary.books_read) * 100)}%`
					: '—',
			icon: '📊'
		},
		{
			label: '起始年份',
			value: summary?.reading_period.earliest_date?.slice(0, 4) || '—',
			icon: '🗓️'
		}
	]);
</script>
...
{#if summary}
	<section class="grid grid-cols-2 gap-4 sm:grid-cols-4" aria-label="读书统计">
		{#each items as item}
